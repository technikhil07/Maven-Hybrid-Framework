...
public void ERP_Test()throws Throwable
{
	...
	if(xl.getCellData("MasterTestCases", i, 2).equalsIgnoreCase("Y"))
	{
		...
		//genrate user define html report
		report = new ExtentReports("./Reports//"+TCModule+FunctionLibrary.genrateDate()+".html");
		...
		//read all columns from TC Module
			...
			}catch(Exception e)
			{
				...
				FileUtils.copyFile(screen, new File("./Screens//"+TCModule+FunctionLibrary.genrateDate()+"Myscreen.png"));
			break;
			}
			report.endTest(test);
			report.flush();
